# -*- coding: utf-8 -*-
"""
Created on Wed Nov 25 16:38:55 2020
...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 31 19:58:09 2020
...
for i in prop:
    if i not in property_list:
        if i not in al:
            ...
            exit()    
# Function to create all possible kmers   
def allp(k):
  n =['A','T','C','G']
  s=[]
  if (k==1):
          ...
          for j in n:
              for k in n:
                  ...
                  if (se not in s):
                      s.append(se)
      return s
# Function to create kmers of given input sequence
def end(seq,n,k):
    i=len(seq)-1
    sq1=""
    for i in range(n):
...
cdk = pd.DataFrame()
# if input sequence is to be read using cmd itself
if(file_extension==""):
    f1=f1.upper()
    alphabet=['A','C','G','T']
    for i in f1:
        if i not in alphabet:
            ...
            exit()
    sq.append(end(f1,n,3))
    cdk['Sequence'] = sq
# if input sequences are to be read using input file  
else:
    f=open(f1,"r")
    b= f.readlines()
    s_id =[]
        ...
        for p1 in range(len(phy)):
            for p2 in range(len(phy)):
                ...
                if(p1!=p2):
                    ...
                    count = count+1
                    sum=0.0
                    st = "TCC_CT_"+ phy[p1]+"_"+phy[p2]+"_lag"+str(l)
                    for i in range(len(seq)-l-2):
