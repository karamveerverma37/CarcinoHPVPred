# -*- coding: utf-8 -*-
"""
Created on Fri Nov 27 23:27:41 2020
...
for k_temp in seq:
    for k_c,sq in enumerate(k_temp):
        pl = allp(2)
        rs = kmer(2,sq,order)
                        ...
                        for j in pl:
                            ...
                            if j==i:
                                ...
                                for l in data['Physicochemical properties']:
                                    ...
                                    if(l==q):
                                        w = q.split('p')
                                        s = "PR"+str(w[1])+"_SPLIT_s"+str(k_c+1)
                                        dict_n[t+"_"+s].append(data[i][c])
                            else:
                                w = q.split('p')
                                s = "PR"+str(w[1])+"_SPLIT_s"+str(k_c+1)
                                dict_n[t+"_"+s].append(0)
                    if count < l_ar[k_c] - 1:
                        while count != l_ar[k_c] - 1:
                            count = count + 1
                            for j in pl:
                                w = q.split('p')
                                t = 'P' + str(count) + '_' + j + '_' + 'PR' + str(w[1]) + '_SPLIT_s' + str(k_c + 1)
                                dict_n[t].append(0)
            else:
                count=0
                for i in rs:
