# -*- coding: utf-8 -*-
"""
Created on Wed Nov 25 16:30:37 2020
...
for i in prop:
    if i not in property_list:
        if i not in al:
            ...
            exit()    
# Function to create all posssible kmers    
def allp(k):
  n =['A','T','C','G']
  s=[]
  if (k==1):
...
def end(seq,n,k):
    ...
    if(len(sq1)<k):
        ...
        sys.exit()
    return sq1
# function to creating kmers for given input sequence
def kmer(k,seq):
  s=[]
...
cdk = pd.DataFrame()
# if input sequence is to be read from cmd itself
if(file_extension==""):
    f1=f1.upper()
    alphabet=['A','C','G','T']
    for i in f1:
    ...
    cdk['Sequence'] = sq
# if input sequences aare to be read from input file 
else:
    f=open(f1,"r")
    b= f.readlines()
    s_id =[]
        ...
        for p1 in range(len(phy)):
            for p2 in range(len(phy)):
                if(p1!=p2):
                    ...
                    count = count+1
                    sum=0.0
                    st = "DCC_CT_"+ phy[p1]+"_"+phy[p2]+"_lag"+str(l)
                    for i in range(len(seq)-l-1):
